...
def coordinated_search(tree, idx, options, head_map):
	subtree = tree.subtrees[idx]
	if get_head(head_map, subtree)[2] in options or subtree.label in options:
		add_head(head_map, tree, get_head(head_map, subtree))
		return True
	return False
...
###	after parsing. The following rules are then used to recover the NP head:
	...
	if (len(tree.subtrees) == 4 and
	    ...
	    tree.subtrees[1].label == ',' and
	    tree.subtrees[2].label == 'NP' and
	    tree.subtrees[3].label == ','):
		if coordinated_search(tree, 0, pos_to_look, head_map):
			return
	# NP -> NP CC NP
	# NP -> NNP CC NNP
	# NP -> NP , NP
        #if tree.word_yield().startswith("Anderson"):
        #    print tree, len(tree.subtrees)
        
	if (len(tree.subtrees) == 3 and
	    tree.subtrees[0].label in {'NP', 'NNP'} and
	    tree.subtrees[1].label in {'CC', ','} and
	    tree.subtrees[2].label in {'NP', 'NNP'}):
		if coordinated_search(tree, 0, pos_to_look, head_map):
			return
	# NP -> NP NP
	if (len(tree.subtrees) == 2 and
	    tree.subtrees[0].label == 'NP' and
	    tree.subtrees[1].label == 'NP'):
		if coordinated_search(tree, 0, pos_to_look, head_map):
			return
	# Canasai's addition end
